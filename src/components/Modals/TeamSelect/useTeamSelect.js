...
const calculateIsDropRequired = () => {
    ...
    //   max teams is number of active slots for next month * 2
      const maxTeams =
        (Object.values(props.currentRoflMonths).filter(
...
const isDropRequired = useMemo(() => { return calculateIsDropRequired()}, [])
  ...
  const topText = {
    submitBid: (
      <TopText>
        If successful, this bid will be available for you starting Rofl month {props.firstActiveMonthForClaim}
        <br/><br/>
        {isDropRequired
    ...
    addTeam: (
      <TopText>
        This claim will be available for you starting Rofl month {props.firstActiveMonthForClaim}
        <br/><br/>
        {isDropRequired
  ...
  const maxCashAllowed = {
    ...
    tradeFor: (
      <>
        <p>{orgMembers[props?.userToTradeWith]?.name}'s cash: ${props.receiverRoster?.cash}</p>
      </>
    ),
    tradeAway: (
      <>
        <p>Your cash: ${props?.currentUserRoster?.cash}</p>
      </>
    )
  };
  ...
  console.log('here is modal title before return')
